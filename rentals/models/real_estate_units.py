# rentals/models/real_estate_units.py
...
class RealEstateUnit(models.Model):
    building = models.ForeignKey(
        ...
        related_name='real_estate_units',  # ✅ Renommé
    ...
    unit_type = models.CharField(  # ✅ Renommé (ex-property_type)
        max_length=20, choices=UnitType.choices,
        verbose_name=_("Unit Type")
    )
    unit_number = models.CharField(  # ✅ Gardé (déjà clair)
        max_length=50, verbose_name=_("Unit Number"),
        help_text=_("Apartment number, room number, or another identifier.")
    )
    ...
    class Meta:
        verbose_name = _("Real Estate Unit")  # ✅ Renommé
        verbose_name_plural = _("Real Estate Units")  # ✅ Renommé
        ordering = ['building__name', 'unit_number']


    def __str__(self):
        return f"{self.get_unit_type_display()} {self.unit_number} ({self.building.name})"  # ✅ Mis à jour
