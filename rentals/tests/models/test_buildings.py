...
from rentals.models import Building, Owner, Property
...
class TestBuildingModel:
    ...
    def test_available_properties_count_with_no_properties(self):
        ...
        assert building.available_properties_count == 0
        assert building.total_properties_count == 0
        assert building.rented_properties_count == 0
    ...
    def test_available_properties_count_with_properties(self):
        ...
        assert building.available_properties_count == 1
        assert building.total_properties_count == 2
        assert building.rented_properties_count == 1
    ...
    def test_building_with_annotations(self):
        ...
        buildings = Building.objects.annotate(
            available_count=Count('properties', filter=models.Q(properties__is_available=True)),
            total_count=Count('properties'),
            rented_count=Count('properties', filter=models.Q(properties__is_available=False))
        )
        annotated_building = buildings.get(pk=building.pk)
        assert annotated_building.available_count == 1
        assert annotated_building.total_count == 2
        assert annotated_building.rented_count == 1
